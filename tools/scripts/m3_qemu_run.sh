...
OVMF_VARS_RUN="${OVMF_VARS_RUN:-/tmp/mcsos_ovmf_vars_$$.fd}"

fail() { echo "FAIL: $*" >&2; exit 1; }

test -f "$ISO"                || fail "ISO tidak ditemukan: $ISO"
test -f "$OVMF_CODE"          || fail "OVMF_CODE tidak ditemukan: $OVMF_CODE"
test -f "$OVMF_VARS_TEMPLATE" || fail "OVMF_VARS template tidak ditemukan: $OVMF_VARS_TEMPLATE"
command -v qemu-system-x86_64 >/dev/null 2>&1 || fail "qemu-system-x86_64 tidak ditemukan"

# Copy fresh OVMF_VARS agar boot order tidak tersimpan dari run sebelumnya
